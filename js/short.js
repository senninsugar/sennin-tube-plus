...
window.__gcse = {
  ...
  initializationCallback: function() {
    ...
    // Shorts検索ページかどうかを判定
    // ページネーション時は results-containerが既に存在するため、アクセス時のみ初期検索
    if ($("#results-container.shorts-grid").length > 0) {
      isShortSearchPage = true;
      // ページ１でかつ検索語句がある場合のみCSE検索を実行
      if (currentPage === 1 && currentSearchQuery && currentSearchQuery.trim().length > 0) {
        // 1ページ目で結果が空の場合のみ CSE 経由で検索
        if ($("#results-container").find('.video-card.is-short').length === 0) {
          triggerCseShortsSearch(currentSearchQuery);
        }
      }
    }
  },
    ...
    web: {
      ready: function(name, q, promos, results) {
        ...
        if (isShortSearchPage && $("#results-container.shorts-grid").length > 0) {
          renderShortsGridResults(videos);
        } else {
          // ホーム画面での描画
...
function loadNextShortsPage() {
  ...
  // currentPageは search.htmlで定義されたグローバル変数を使用
...
function initShortsInfiniteScroll() {
  $(window).on('scroll', function () {
    ...
    if (scrollTop + windowHeight >= documentHeight - 300) {
      // currentPage は search.html で定義されているグローバル変数を参照
      loadNextShortsPage();
    }
  });
}
